...
qssSCALED <- function(motMATS){
  ...
  # initiate null matrix list
  matCon.N <- list()
  # initiate data matrix for eigenvalue
  lam.max <- matrix(0, nrow = 10000, ncol = mots)
  # initiate data matrix for eigenvalue of null model
  ...
  qss.2 <- apply(lam.max, 2, function(x){sum(x[1:100] < 0)/length(x[1:100])})
  qss.2.N <- apply(lam.max.N, 2, function(x){sum(x[1:100] < 0)/length(x[1:100])})
...
qssDAT <- data.frame(size = factor(c(rep("4-node", length(qss4)),rep("5-node", length(qss5)),rep("10-node", length(qss10))), 
           ...
           qss2 = c(qss4.2[[1]], qss5.2[[1]], qss10.2[[1]]),
           qss3 = c(qss4.2[[2]], qss5.2[[2]], qss10.2[[2]])
           )
...
```{r}
ggplot(qssDAT, aes(x = qss3, y = qss2)) + geom_point(aes(color = size)) + geom_abline(intercept = 0, slope = 1) + theme_bw()
